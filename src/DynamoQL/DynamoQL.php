...
// ----------------------------------------------------------------------------
{
    ...
    private function prepareReturnValue( $response, $commandType = "" )
    {
        ...
        $r["rows"]     = null;
        $r["code"]    = -1;
        $r["message"]    = "Invalid response. Please review your dql query.";
        $r["exception"]  = "";
        $r["type"]     = $commandType;
        ...
        if ( $response === false ) {
            ...
            $r["code"] = \DynamoQL\Common\Enum\Response::OK;
            $r["message"] = "";
        } else {
            ...
            $r["exception"] = isset($response['exception_type']) ? $response['exception_type'] : '';
        }
    ...
    public function dql( $dql, $try_count = 0 )
    {
        ...
        if ( $r["code"] != \DynamoQL\Common\Enum\Response::OK ) {
            $must_retry     = false;
                ...
                case "insert":
                    switch ($r["code"]) {
                        case \DynamoQL\Common\Enum\Response::CONDITIONAL_CHECK_FAILED:
                            ...
                            $r["message"] = "A value with the given hash key already exists.";
                            break;
                    }
                    break;
                case "q upda":
            ...
            //ExceptionManager::raiseError(new DynamoQLException("DynamoQL: DQL ERROR ".($try_count>=$this->maximum_try_count ? "(maximum [".$this->maximum_try_count."] tries made)" : "")." " . $r["code"]." : ".$r["message"]." > ".$dql));
            ExceptionManager::raiseError(new DynamoQLException($r["message"], $r["code"]));
        }
...
//        }
    ...
    public function parseDQL( $dql )
    {
        ...
        switch (strtoupper(trim(substr($dql, 0, 6)))) {
            ...
            case "SELECT":
                ...
                if ( $cmd["FIELDS"][0] == "*" )
                    $cmd["FIELDS"] = [];
                $cmd["TABLE"]    = $this->prefixTableName(trim($split[3]));
                $cmd["WHERE"]    = "";
                $cmd["LIMIT"]    = "";
                $cmd["STARTING"] = "";
                if ( count($split) > 5 ) {
            ...
            case "INSERT":
                ...
                $cmd["TABLE"] = $this->prefixTableName(trim($split[1]));
                try {
            ...
            case "UPDATE":
                ...
                $cmd["TABLE"] = $this->prefixTableName(trim(trim($split[1])));
                $set_where    = $split[3];
            ...
            case "DELETE":
                ...
                $cmd["TABLE"] = $this->prefixTableName(trim($split[1]));
                $cmd["WHERE"] = $this->processDeleteCondition($cmd["TABLE"], trim($split[3]));
                break;
            case "Q CREA":
            case "CREATE":
    ...
    public function prefixTableName( $table_name )
    {
