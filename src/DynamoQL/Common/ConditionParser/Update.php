        ...
        foreach ( $condition as $condition_array ) {
            ...
            if ( $op == "EQ" ) {
                if ( strtolower($target) == "hash" || strtolower($target) == "hash key" || $target == $keypair->hash->name ) {
                    // hash condition
                    $keys[$keypair->hash->name] = [$keypair->hash->type => $params[0]];
                    ...
                    // range condition
                    $keys[$keypair->range->name] = [$keypair->range->type => $params[0]];
                    //$keys["RangeKeyElement"] = [$keypair->range->type => $params[0]];
                }
                // we treat the special NULL value to match non-existing fields
                if (trim($params[0], "\"'") == "")
                    $expected[$target] = ["Exists" => false];
                else
